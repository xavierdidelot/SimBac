cfml_results=function(prefix,showcols=T,scalingbar=NA) {
### Read a FASTA file
...
if (!is.na(scalingbar)) tree$edge.length=tree$edge.length*scalingbar

# Load a list cross-referencing patterns in the original data to the output FASTA file
xref = scan(xreffile,sep=",",quiet=T)
...
ML_seq=scan(ML_seqfile,what=character(0),quiet=T)
...
new_plot.phylo = get("last_plot.phylo", envir = .PlotPhyloEnv); new_plot.phylo$yy = rank(vpos,ties.method='first')[rev_M_node_index]#XD added ties.method='first' on 14/5/2021
...
#axisPhylo()
if (!is.na(scalingbar)) add.scale.bar()
...
if (showcols) alt.image(spectrum.mut[,od],col=c("skyblue","white","yellow",colorRampPalette(c("orange","red"))(pmax(0,max(spectrum.mut)-2))),xlab="",ylab="",axes=FALSE,xaxs="i",yaxs="i",xpos=which(xref>0),background.fun=background.noncore)
else alt.image(spectrum.mut[,od],col=c("white","white","white",colorRampPalette(c("white","white"))(pmax(0,max(spectrum.mut)-2))),xlab="",ylab="",axes=FALSE,xaxs="i",yaxs="i",xpos=which(xref>0),background.fun=background.noncore)
axis(1); axis(2,1:nrow(wh.mut),ifelse((1:nrow(wh.mut))<=ceiling(nrow(wh.mut)/2),rownames(wh.mut),"")[od],las=2,cex.axis=0.8); box()
# Plot the recombination intervals
if(!is.na(istatefile)) {
